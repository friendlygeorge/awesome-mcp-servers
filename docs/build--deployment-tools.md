...
- [friendlygeorge/mcp-server-terraform](https://github.com/friendlygeorge/mcp-server-terraform): Full Terraform CLI integration with 10 tools (plan, apply, destroy, init, validate, state management, fmt, output, import) for infrastructure-as-code automation via MCP.
