...
- [friendlygeorge/cron-scheduler-mcp-server](https://github.com/friendlygeorge/cron-scheduler-mcp-server): Cron job scheduling MCP server with SQLite persistence, retry logic, and systemd integration. 10 tools for creating, managing, and monitoring scheduled tasks. MIT license. Install: `npx @supernova123/cron-scheduler-mcp-server`. No API keys required.
